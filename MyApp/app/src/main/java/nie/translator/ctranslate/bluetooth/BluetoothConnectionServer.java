...
class BluetoothConnectionServer extends nie.translator.ctranslate.bluetooth.BluetoothConnection {
    //costants
    ...
    public BluetoothConnectionServer(final Context context, final String name, @NonNull final BluetoothAdapter bluetoothAdapter, final int strategy, final BluetoothConnectionClient client, final Callback callback) {
                        ...
                        if (newState == BluetoothProfile.STATE_CONNECTED) {
                            ...
                            synchronized (channelsLock) {
                                ...
                                if (!client.getConnectedPeers().contains(peer) && !channels.contains(peer)) {   // the client object is used to manage synchronization with the client to avoid adding a device that connects to the latter instead of us
                                    channels.add(new nie.translator.ctranslate.bluetooth.ServerChannel(context, peer, bluetoothAdapter));
                                    index = channels.size() - 1;
                                    ((nie.translator.ctranslate.bluetooth.ServerChannel) channels.get(index)).setBluetoothGattServer(bluetoothGattServer);
                            ...
                            synchronized (channelsLock) {
                                ...
                                if (index != -1) {
                                    ((nie.translator.ctranslate.bluetooth.ServerChannel) channels.get(index)).setBluetoothGattServer(null);
                        ...
                        synchronized (channelsLock) {
                            ...
                            if (characteristic.getUuid().equals(CONNECTION_REQUEST_UUID)) {
                                ...
                                if (index != -1) {
                                    if (!channels.get(index).getPeer().isDisconnecting()) {
                                        if (channels.get(index).getPeer().isReconnecting()) {
                                            if (!((nie.translator.ctranslate.bluetooth.ServerChannel) channels.get(index)).notifyConnectionResumed()) {
                                                stopReconnection(channels.get(index));
                                            }
                                            ...
                                            if (!((nie.translator.ctranslate.bluetooth.ServerChannel) channels.get(index)).notifyConnectionResumedRejected()) {
                                ...
                                if (index != -1) {
                                    if (!channels.get(index).getPeer().isDisconnecting()) {
                                        try {
                                            ...
                                            BluetoothGattService service = bluetoothGattServer.getService(nie.translator.ctranslate.bluetooth.BluetoothConnection.APP_UUID);
                                ...
                                if (index != -1) {
                                    ...
                                    nie.translator.ctranslate.bluetooth.BluetoothMessage subMessage = nie.translator.ctranslate.bluetooth.BluetoothMessage.createFromBytes(context, sender, value);
                                    if (subMessage != null) {
                                ...
                                if (index != -1) {
                                    Peer sender = (Peer) channels.get(index).getPeer().clone();
                                    nie.translator.ctranslate.bluetooth.BluetoothMessage subData = nie.translator.ctranslate.bluetooth.BluetoothMessage.createFromBytes(context, sender, value);
                                    if (subData != null) {
                ...
                mainHandler.post(new Runnable() {
                    ...
                    public void run() {
                        synchronized (channelsLock) {
                            ...
                            if (index != -1) {
                                UUID sendingCharacteristic = ((nie.translator.ctranslate.bluetooth.ServerChannel) channels.get(index)).getSendingCharacteristic();
        ...
        BluetoothGattService service = new BluetoothGattService(nie.translator.ctranslate.bluetooth.BluetoothConnection.APP_UUID, BluetoothGattService.SERVICE_TYPE_PRIMARY);
        ...
        mainHandler.post(new Runnable() {
            ...
            public void run() {
                synchronized (channelsLock) {
                    ...
                    if (index != -1) {
                        if (!channels.get(index).getPeer().isConnected() && !channels.get(index).getPeer().isReconnecting()) {
                            if (!((nie.translator.ctranslate.bluetooth.ServerChannel) channels.get(index)).acceptConnection()) {
                                channels.get(index).disconnect(disconnectionCallback);
                            }
                        }
                    }
            ...
            public void run() {
                synchronized (channelsLock) {
                    ...
                    if (index != -1) {
                        ...
                        if (!channels.get(index).getPeer().isConnected() && !channels.get(index).getPeer().isReconnecting()) {
                            channels.get(index).getPeer().setDisconnecting(true);
                            if (!((nie.translator.ctranslate.bluetooth.ServerChannel) channels.get(index)).rejectConnection()) {
                                channels.get(index).disconnect(disconnectionCallback);
                            }
                        }
                    }
