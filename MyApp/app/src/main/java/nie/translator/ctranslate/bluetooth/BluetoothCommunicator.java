...
public class BluetoothCommunicator {
    ...
    private ArrayDeque<nie.translator.ctranslate.bluetooth.Message> pendingMessages = new ArrayDeque<>();
    private ArrayDeque<nie.translator.ctranslate.bluetooth.Message> pendingData = new ArrayDeque<>();
    // objects
    private Context context;
    @Nullable
    private BluetoothAdapter bluetoothAdapter;
    @Nullable
    private nie.translator.ctranslate.bluetooth.BluetoothConnectionServer connectionServer;
    @Nullable
            ...
            public synchronized void onScanResult(int callbackType, ScanResult result) {
                ...
                switch (callbackType) {
                    case ScanSettings.CALLBACK_TYPE_ALL_MATCHES: {
                        ...
                        if (result.getScanRecord() != null && connectionClient != null) {
                            ...
                            if (uniqueName != null && uniqueName.length() > 0) {
                                nie.translator.ctranslate.bluetooth.Peer peerFound = new nie.translator.ctranslate.bluetooth.Peer(device1, uniqueName, false);
                    ...
                    case ScanSettings.CALLBACK_TYPE_MATCH_LOST: {
                        BluetoothDevice device1 = result.getDevice();
                        notifyPeerLost(new nie.translator.ctranslate.bluetooth.Peer(device1, null, false));
                        break;
                    }
                }
            }
    ...
    private void initializeConnection() {
        if (bluetoothAdapter != null) {
            nie.translator.ctranslate.bluetooth.BluetoothConnection.Callback connectionCallback = new nie.translator.ctranslate.bluetooth.BluetoothConnection.Callback() {
                @Override
                public void onConnectionRequest(nie.translator.ctranslate.bluetooth.Peer peer) {
                    super.onConnectionRequest(peer);
                    notifyConnectionRequest(peer);
                }

                @Override
                public void onConnectionSuccess(nie.translator.ctranslate.bluetooth.Peer peer, int source) {
                    super.onConnectionSuccess(peer, source);
                    notifyConnectionSuccess(peer, source);
                }

                @Override
                public void onConnectionLost(nie.translator.ctranslate.bluetooth.Peer peer) {
                    super.onConnectionLost(peer);
                ...
                @Override
                public void onPeerUpdated(nie.translator.ctranslate.bluetooth.Peer peer, nie.translator.ctranslate.bluetooth.Peer newPeer) {
                    super.onPeerUpdated(peer, newPeer);
                    notifyPeerUpdated(peer, newPeer);
                }

                @Override
                public void onConnectionResumed(nie.translator.ctranslate.bluetooth.Peer peer) {
                    super.onConnectionResumed(peer);
                    synchronized (BluetoothCommunicator.this) {
                        if (connectionServer != null && connectionClient != null) {
                            if (connectionServer.getReconnectingPeers().size() == 0 && !advertising) {
                ...
                @Override
                public void onConnectionFailed(nie.translator.ctranslate.bluetooth.Peer peer, int errorCode) {
                    super.onConnectionFailed(peer, errorCode);
                    notifyConnectionFailed(peer, errorCode);
                }

                @Override
                public void onMessageReceived(nie.translator.ctranslate.bluetooth.Message message, int source) {
                    super.onMessageReceived(message, source);
                ...
                @Override
                public void onDataReceived(nie.translator.ctranslate.bluetooth.Message data, int source) {
                ...
                @Override
                public void onDisconnected(nie.translator.ctranslate.bluetooth.Peer peer) {
            ...
            connectionServer = new nie.translator.ctranslate.bluetooth.BluetoothConnectionServer(context, uniqueName, bluetoothAdapter, strategy, connectionClient, connectionCallback);
        }
    }
    ...
    private int executeStartAdvertising() {
        ...
        if (advertisementSupportedCode == SUCCESS) {
            ...
            AdvertiseData advertiseData = new AdvertiseData.Builder()
                    .addServiceUuid(new ParcelUuid(nie.translator.ctranslate.bluetooth.BluetoothConnection.APP_UUID))
    ...
    private int executeStartDiscovery() {
        if (bluetoothAdapter != null) {
            ...
            scanFilters.add(new ScanFilter.Builder()
                    .setServiceUuid(new ParcelUuid(nie.translator.ctranslate.bluetooth.BluetoothConnection.APP_UUID))
                    .build());
     ...
     */
    public void sendMessage(final nie.translator.ctranslate.bluetooth.Message message) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
    ...
    private void sendMessage() {
        if (connectionClient != null && connectionServer != null) {
            final nie.translator.ctranslate.bluetooth.Message message = pendingMessages.peekFirst();
            if (message != null) {
                connectionClient.sendMessage(message, new nie.translator.ctranslate.bluetooth.Channel.MessageCallback() {
                    @Override
                    public void onMessageSent() {
                        connectionServer.sendMessage(message, new nie.translator.ctranslate.bluetooth.Channel.MessageCallback() {
                            @Override
     ...
     */
    public void sendData(final nie.translator.ctranslate.bluetooth.Message data) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
    ...
    private void sendData() {
        if (connectionClient != null && connectionServer != null) {
            final nie.translator.ctranslate.bluetooth.Message data = pendingData.peekFirst();
            if (data != null) {
                connectionClient.sendData(data, new nie.translator.ctranslate.bluetooth.Channel.MessageCallback() {
                    @Override
                    public void onMessageSent() {
                        connectionServer.sendData(data, new nie.translator.ctranslate.bluetooth.Channel.MessageCallback() {
                            @Override
                            public void onMessageSent() {   // means that we have sent the message to all the client and server channels
     ...
     */
    public ArrayList<nie.translator.ctranslate.bluetooth.Peer> getConnectedPeersList() {
        ArrayList<nie.translator.ctranslate.bluetooth.Peer> connectedPeers = new ArrayList<>();
        if (connectionServer != null && connectionClient != null) {
     ...
     * @return SUCCESS if bluetooth le is supported by the device or BLUETOOTH_LE_NOT_SUPPORTED if not (or rarely if we had a generic bluetooth problem)
     */
    public int acceptConnection(nie.translator.ctranslate.bluetooth.Peer peer) {
        if (connectionServer != null) {
            connectionServer.acceptConnection((nie.translator.ctranslate.bluetooth.Peer) peer.clone());
            return SUCCESS;
        } else if(isBluetoothLeSupported() == BLUETOOTH_LE_NOT_SUPPORTED){
            return BLUETOOTH_LE_NOT_SUPPORTED;
        }else{
     ...
     * @return SUCCESS if bluetooth le is supported by the device or BLUETOOTH_LE_NOT_SUPPORTED if not (or rarely if we had a generic bluetooth problem)
     */
    public int rejectConnection(nie.translator.ctranslate.bluetooth.Peer peer) {
        if (connectionServer != null) {
            connectionServer.rejectConnection((nie.translator.ctranslate.bluetooth.Peer) peer.clone());
            return SUCCESS;
        } else if(isBluetoothLeSupported() == BLUETOOTH_LE_NOT_SUPPORTED){
            return BLUETOOTH_LE_NOT_SUPPORTED;
        }else{
     ...
     */
    public int connect(final nie.translator.ctranslate.bluetooth.Peer peer) {
        if (!destroying) {
            if (connectionClient != null) {
                connectionClient.connect((nie.translator.ctranslate.bluetooth.Peer) peer.clone());
                return SUCCESS;
            } else if(isBluetoothLeSupported() == BLUETOOTH_LE_NOT_SUPPORTED) {
                return BLUETOOTH_LE_NOT_SUPPORTED;
            } else {
    ...
    public int readPhy(nie.translator.ctranslate.bluetooth.Peer peer) {
        if (connectionClient != null && connectionServer != null) {
            connectionServer.readPhy((nie.translator.ctranslate.bluetooth.Peer) peer.clone());
            connectionClient.readPhy((nie.translator.ctranslate.bluetooth.Peer) peer.clone());
            return SUCCESS;
        } else if(isBluetoothLeSupported() == BLUETOOTH_LE_NOT_SUPPORTED){
            return BLUETOOTH_LE_NOT_SUPPORTED;
        }else{
     ...
     * @return SUCCESS if bluetooth le is supported by the device or BLUETOOTH_LE_NOT_SUPPORTED if not (or rarely if we had a generic bluetooth problem)
     */
    public int disconnect(final nie.translator.ctranslate.bluetooth.Peer peer) {
        synchronized (bluetoothLock) {
            if (connectionClient != null && connectionServer != null && bluetoothAdapter != null) {
                connectionServer.disconnect((nie.translator.ctranslate.bluetooth.Peer) peer.clone());
                connectionClient.disconnect((nie.translator.ctranslate.bluetooth.Peer) peer.clone());
    ...
    public int disconnectFromAll() {
        if (connectionClient != null && connectionServer != null) {
            connectionServer.disconnectAll(new nie.translator.ctranslate.bluetooth.Channel.DisconnectionNotificationCallback() {
                @Override
    ...
    private void notifyPeerFound(final nie.translator.ctranslate.bluetooth.Peer peer) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyPeerLost(final nie.translator.ctranslate.bluetooth.Peer peer) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyConnectionRequest(final nie.translator.ctranslate.bluetooth.Peer peer) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyConnectionSuccess(final nie.translator.ctranslate.bluetooth.Peer peer, final int source) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyConnectionFailed(final nie.translator.ctranslate.bluetooth.Peer peer, final int errorCode) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyConnectionResumed(final nie.translator.ctranslate.bluetooth.Peer peer) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyConnectionLost(final nie.translator.ctranslate.bluetooth.Peer peer) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyMessageReceived(final nie.translator.ctranslate.bluetooth.Message message, final int source) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyPeerUpdated(final nie.translator.ctranslate.bluetooth.Peer peer, final nie.translator.ctranslate.bluetooth.Peer newPeer) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyDisconnection(final nie.translator.ctranslate.bluetooth.Peer peer, final int peersLeft) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    public static abstract class Callback extends nie.translator.ctranslate.bluetooth.BluetoothConnection.Callback {
        /**
         * Notify that advertise has started, if you want to do something after the start of advertising do it here, because
         * after startAdvertise there is no guarantee that advertise is really started (it is delayed)
         */
         ...
         */
        public void onPeerFound(nie.translator.ctranslate.bluetooth.Peer peer) {
        }
         ...
         * @param peer
         */
        public void onPeerLost(nie.translator.ctranslate.bluetooth.Peer peer) {
        }
